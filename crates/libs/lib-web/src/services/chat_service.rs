...
use lib_core::{
    ctx::Ctx,
    model::{
        chat::{ChatForCreate, ChatForSelect, ChatRepo},
...
impl ChatService {
    pub async fn get_chats(mm: Arc<ModelManager>, ctx: Ctx) -> Result<Vec<ChatDto>> {
        ...
        let chats = user_chat_members.into_iter().map(|cm| {
            ...
            async move {
                ...
                Self::converte_chat_to_dto(mm, ctx, chat).await
            }
        });
    ...
    pub async fn get_unread_count(mm: Arc<ModelManager>, ctx: Ctx) -> Result<u32> {
        let user_id = ctx.user_id.ok_or(Error::Unauthorized)?;
    ...
    pub async fn has_chat_with_user(
        ...
        for chat_id in common_chat_ids {
            ...
            if !chat.is_group {
                return Ok(Some(Self::converte_chat_to_dto(mm, ctx, chat).await?));
            }
        }
    ...
    pub async fn get_members(mm: Arc<ModelManager>, ctx: Ctx, chat_id: &Uuid) -> Result<Vec<Uuid>> {
        let _ = ctx.user_id.ok_or(Error::Unauthorized);
    ...
    pub async fn converte_chat_to_dto(
        mm: Arc<ModelManager>,
        ctx: Ctx,
        chat: ChatRepo,
    ) -> Result<ChatDto> {
        let reqeuster_id = ctx.user_id.ok_or(Error::Unauthorized)?;
        let unread_count = Self::get_unread_count(mm.clone(), ctx.clone()).await?;
        let last_message = Self::get_last_message(mm.clone(), ctx.clone(), &chat.id).await?;
        let user_id = Self::get_members(mm.clone(), ctx.clone(), &chat.id)
            .await?
            .into_iter()
            .filter(|uid| uid.ne(&reqeuster_id))
            .next()
            .ok_or(Error::UserNotFound)?;

        let username = UserService::get_by_id(mm.db(), ctx.user_id, &user_id)
            .await?
            .nickname;
